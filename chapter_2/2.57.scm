...
(define (multiplier p) (cadr p))  ; first term

; multiplication of the rest of the terms
(define (multiplicand p)
    (define (handle seq result)
        (cond ((pair? seq)
                (handle (cdr seq)
                        (make-product result (car seq))))
              (else result)))
    (handle (cdr (cdr p)) (make-product 1 1)))
